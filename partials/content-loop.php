...
if ( 'link' === $post_type ) {
	...
	$host                     = wp_parse_url( get_the_permalink(), PHP_URL_HOST );
...
if ( ! empty( $terminal_card_title ) ) {
	printf(
		...
		! empty( $terminal_card_title_icon ) ?
			sprintf(
				'<div class="terminal-card-icon"><svg-icon><src href="%s" /></svg-icon></div>',
				esc_url( $terminal_card_title_icon )
			) :
			'',
...
printf(
	...
	esc_attr( $hide_excerpt_on_mobile ? 'terminal-mobile-hide' : '' ),
