 ...
 */
function terminal_broadstreet_ad( $height, $width, $regular_unit, $amp_unit, $header = false, $amp_disable = false ) {
	if ( ! is_amp_endpoint() ) {
		if ( ! empty( $header ) ) {
			return sprintf(
				'<div class="terminal-inline-ad"><div class="terminal-ad-header">%s</div><broadstreet-zone zone-id="%s"></broadstreet-zone></div>',
				esc_html( $header ),
				esc_attr( $regular_unit )
			);
		}
		return sprintf(
			'<broadstreet-zone zone-id="%s"></broadstreet-zone>',
			esc_attr( $regular_unit )
		);
	}
	if ( $amp_disable ) {
	...
	if ( ! empty( $header ) ) {
		return sprintf(
			'<div class="terminal-inline-ad"><div class="terminal-ad-header">%s</div><amp-ad width="%s" height="%s" type="broadstreetads" data-network="5918" data-zone="%s"></amp-ad></div>',
			esc_html( $header ),
			esc_attr( $width ),
			esc_attr( $height ),
			esc_attr( $amp_unit )
