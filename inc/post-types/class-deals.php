...
class Deals {
	...
	public function filter_feed_author_link( $link ) {
		$id = get_the_id();
		if ( get_post_type( $id ) === $this->deal_post_type ) {
			$url = get_post_meta( $id, $this->deal_post_type_link_key, true );
			if ( ! empty( $url ) ) {
				return $url;
			}
		}
	...
	public function filter_feed_title( $title, $id = null ) {
		...
		if ( $this->deal_post_type === get_post_type( $id ) ) {
			return "🛍 ${title}";
		}
		return $title;
	}
	...
	public function change_headline_to_deal_title( $title ) {
		...
		if ( $this->deal_post_type === $screen->post_type ) {
